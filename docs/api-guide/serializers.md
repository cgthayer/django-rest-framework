...
For views, it's often useful to override `get_serializer_context` to add any additional info needed by your serializer to the context dict.  For example, here's a serializer that looks at the captured values from url, which come to the view as self.kwargs (keyword arguments to the constructor).
